...
EightShapes 共同創業者。デザインシステム分野で最も引用される書き手の一人で、デザイントークンの命名体系・空間設計・文書化・チームモデルに関する体系的な論考で知られる。

## 主要な事実

- 2016年にトークン論の代表的記事を書き、2020年の「Naming Tokens in Design Systems」で命名レベルの4グループ（Base / Modifier / Object / Namespace）を定式化。Discovery Education、Morningstar、REI、USAC、NetApp など10超のシステムでトークン設計に関与（出典: [[Naming Tokens in Design Systems]]）
- 「**プリミティブトークンは選択肢（options）、セマンティックトークンは選択（choices）**」の定式化で広く引用される（出典: [[When “semantic tokens” are no longer semantic]]）
...
- 2024年「The Fallacy of Federated Design Systems」で「**連合型を優先したことは間違いでした**」と自己訂正。80超のコンサルティング経験から中央投資なしの連合型成功を「ゼロパーセント」と断じ、「選ぶのではなく、徐々に追加する」へ枠組みを更新した（出典: [[翻訳記事：連合型デザインシステムの誤り｜Nobuya Sato]]）。詳細は [[連合型デザインシステム]]
- 連合型オーナーシップモデルの失敗を語った当人としても引かれる（出典: [[翻訳記事：石油タンカーを方向転換する難しさ｜Nobuya Sato]]）
