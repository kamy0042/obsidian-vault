...
Finally, run the [CLI helper tool](https://github.com/prettier/eslint-config-prettier#cli-helper-tool) to find problems in the `"rules"` section of your `.eslintrc.*` file. (Remember, `"rules"` always “wins” over `"extends"`!)
...
> 
> [eslint-plugin-prettier’s recommended config](https://github.com/prettier/eslint-plugin-prettier#recommended-configuration)
...
- Some can cause problems if using [eslint-plugin-prettier](https://github.com/prettier/eslint-plugin-prettier) and `-fix`.
...
### [arrow-body-style](https://eslint.org/docs/rules/arrow-body-style) and [prefer-arrow-callback](https://eslint.org/docs/rules/prefer-arrow-callback)

**These rules might cause problems if using **[**eslint-plugin-prettier**](https://github.com/prettier/eslint-plugin-prettier)** and **`**--fix**`**.**

See the [`arrow-body-style`](https://github.com/prettier/eslint-plugin-prettier#arrow-body-style-and-prefer-arrow-callback-issue)[ and ](https://github.com/prettier/eslint-plugin-prettier#arrow-body-style-and-prefer-arrow-callback-issue)[`prefer-arrow-callback`](https://github.com/prettier/eslint-plugin-prettier#arrow-body-style-and-prefer-arrow-callback-issue)[ issue](https://github.com/prettier/eslint-plugin-prettier#arrow-body-style-and-prefer-arrow-callback-issue) for details.

There are a couple of ways to turn these rules off:

- Put `"plugin:prettier/recommended"` in your `"extends"`. That’s [eslint-](https://github.com/prettier/eslint-plugin-prettier#recommended-configuration)[**plugin**](https://github.com/prettier/eslint-plugin-prettier#recommended-configuration)[prettier’s recommended config](https://github.com/prettier/eslint-plugin-prettier#recommended-configuration).
...
These rules are safe to use if you don’t use [eslint-plugin-prettier](https://github.com/prettier/eslint-plugin-prettier). In other words, if you run `eslint --fix` and `prettier --write` as separate steps.
...
(The following applies to [vue/max-len](https://github.com/vuejs/eslint-plugin-vue/blob/master/docs/rules/max-len.md) as well.)
...
(The following applies to [babel/quotes](https://github.com/babel/eslint-plugin-babel#rules) and [@typescript-eslint/quotes](https://github.com/typescript-eslint/typescript-eslint/blob/master/packages/eslint-plugin/docs/rules/quotes.md) as well.)
...
- The first (string) option needs to be set to `"single"` or `"double"` and be kept in sync with Prettier’s [singleQuote](https://prettier.io/docs/en/options.html#quotes) option.
- The second (object) option needs the following properties: 
    - `"avoidEscape": true` to follow Prettier’s [string formatting rules](https://prettier.io/docs/en/rationale.html#strings).
...
Ending up with an accidental sequence expression can easily happen while refactoring. If you want ESLint to catch such mistakes, it is recommended to forbid sequence expressions entirely using [no-restricted-syntax](https://eslint.org/docs/rules/no-restricted-syntax) ([as mentioned in the ](https://eslint.org/docs/rules/no-sequences#when-not-to-use-it)[`no-sequences`](https://eslint.org/docs/rules/no-sequences#when-not-to-use-it)[ documentation](https://eslint.org/docs/rules/no-sequences#when-not-to-use-it)):
...
See [package.json](https://github.com/prettier/eslint-config-prettier/blob/main/package.json) for the exact versions of ESLint, Prettier and ESLint plugins that eslint-config-prettier has been tested with.
...
[MIT](https://github.com/prettier/eslint-config-prettier/blob/main/LICENSE).