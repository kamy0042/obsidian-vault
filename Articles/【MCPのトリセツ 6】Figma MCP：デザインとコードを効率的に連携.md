...
![](https://storage.googleapis.com/zenn-user-upload/topics/ea53fb4cd2.png)

![](https://storage.googleapis.com/zenn-user-upload/topics/59acb4066e.png)

![](https://storage.googleapis.com/zenn-user-upload/topics/c7f86c530a.png)
...
1. [MCPの概要と導入方法](https://zenn.dev/takna/articles/mcp-server-tutorial-01-install)
2. [Filesystem MCP Server： AIでローカルファイルを扱う](https://zenn.dev/takna/articles/mcp-server-tutorial-02-filesystem)
3. [YouTube MCPサーバー：動画の内容を取得](https://zenn.dev/takna/articles/mcp-server-tutorial-03-youtube)
4. [mcp-pandoc： AIでドキュメント形式を変換](https://zenn.dev/takna/articles/mcp-server-tutorial-04-pandoc)
5. [GitHub MCPサーバー： AIでリポジトリを管理](https://zenn.dev/takna/articles/mcp-server-tutorial-05-github)
6. 👉 [Figma MCP：デザインとコードを効率的に連携](https://zenn.dev/takna/articles/mcp-server-tutorial-06-figma)
7. [Slack MCPサーバー：チームコミュニケーションを強化](https://zenn.dev/takna/articles/mcp-server-tutorial-07-slack)
8. [Firecrawl MCP：スクレイピングでウェブ情報を取得・分析](https://zenn.dev/takna/articles/mcp-server-tutorial-08-firecrawl)
9. [Markdownify MCP Server: WebページやPDFをMarkdown文書化](https://zenn.dev/takna/articles/mcp-server-tutorial-09-markdownfy)
10. [Raindrop.io MCP Server: 便利なブックマークサービスをAIから使う](https://zenn.dev/takna/articles/mcp-server-tutorial-10-raindropio)
11. [Fetch MCP Server: ウェブコンテンツを取得・処理](https://zenn.dev/takna/articles/mcp-server-tutorial-11-fetch)
12. [Blender MCP Server: 会話で Blender を操作し3Dモデルを作成](https://zenn.dev/takna/articles/mcp-server-tutorial-12-blender)
13. [Perplexity MCP Server: Perplexity ならではの検索をAIとの会話で実行](https://zenn.dev/takna/articles/mcp-server-tutorial-13-perplexity)

参考: [ウェブの情報を取得するMCPの使い分け (Fetch、Firecrawl、Markdownify)](https://zenn.dev/takna/articles/mcp-server-tutorial-reference-web-mcp)
...
14. **正確なデザイン情報の取得**：色、サイズ、フォント、間隔などの正確な数値を取得
15. **コンポーネントデータの抽出**：ボタン、カード、ナビゲーションなどのコンポーネント情報を分析
16. **デザインからコードに変換**：Figmaデザインを対応するHTML/CSS/JavaScriptやReactコンポーネントに変換
17. **デザイントークンの抽出**：カラーパレット、タイポグラフィ、スペーシングなどのデザイントークンを一括取得
18. **レスポンシブデザインの分析**：異なるブレークポイントでのレイアウト変化を理解
19. **デザインコメントの参照**：Figma ファイル内のコメントやフィードバックを参照しながら実装
...
20. [Figma](https://www.figma.com/)にログイン
21. 右上のアイコンから「Settings」を選択
22. 左側メニューから「Account」タブを選択
23. 下部の「Personal access tokens」セクションで「Create a new personal access token」をクリック
24. 適当なトークンの名前（例：`figma-mcp-2025-03-01`）を入力し、「Create token」をクリック
25. 表示されたAPIキーをコピー（**注意**: このキーは一度しか表示されないので安全な場所に保存）
...
26. `~/.codeium/windsurf/mcp_config.json` ファイルを開き、同様の設定を追加します。
...
27. **Figmaデザインファイルを開く**
28. **対象の要素を選択する**（例：「ホーム」フレーム内の「Hero」セクション）
29. **ノードIDの確認方法**： 
...
次回の記事では、Slackでの投稿や情報の取得ができる「[Slack MCPサーバー](https://zenn.dev/takna/articles/mcp-server-tutorial-07-slack)」について解説します。お楽しみに！