...
[Will McGugan@willmcgugan](https://twitter.com/willmcgugan/status/1423678688802058244)

[Absolutely. The thing about semver major version numbers are that they don't mean new stuff, they're a permanent reminder of how many times you got the API wrong. Semver doesn't mean MAJOR.MINOR.PATCH, it means FAILS.FEATURES.BUGS](https://twitter.com/willmcgugan/status/1423678688802058244)
...
[Dominik 🇺🇦@TkDodo](https://twitter.com/TkDodo/status/1647341498227097600)

[📢 We will very likely be removing the onSuccess / onError / onSettled callbacks from `useQuery` in v5. I tried very hard to come up with a good use-case, but imo there is none.](https://twitter.com/TkDodo/status/1647341498227097600)

[I'll write an RFC soon, but if you have a legit use-case, let me know now!](https://twitter.com/TkDodo/status/1647341498227097600)
...
[Michael Carniato@michaelc_1991](https://twitter.com/michaelc_1991/status/1647540073100091393)

[I had an initial knee jerk reaction of thinking this was a bad move. But after reading a lot of the reasons as to why keeping them is a bad move. I'm on board with this now. It makes sense](https://twitter.com/michaelc_1991/status/1647540073100091393)
...
```plain text
1export function useTodos() {2  const query = useQuery({3    queryKey: ['todos', 'list'],4    queryFn: fetchTodos,5  })67  React.useEffect(() => {8    if (query.error) {9      toast.error(query.error.message)10    }11  }, [query.error])1213  return query14}
```
...
```plain text
1const queryClient = new QueryClient({2  queryCache: new QueryCache({3    onError: (error, query) => {4      if (query.meta.errorMessage) {5        toast.error(query.meta.errorMessage)6      }7    },8  }),9})1011export function useTodos() {12  return useQuery({13    queryKey: ['todos', 'list'],14    queryFn: fetchTodos,15    meta: {16      errorMessage: 'Failed to fetch todos',17    },18  })19}
```
...
```plain text
1export function useTodos() {2  const [todoCount, setTodoCount] = React.useState(0)3  const { data: todos } = useQuery({4    queryKey: ['todos', 'list'],5    queryFn: fetchTodos,6    //😭 please don't7    onSuccess: (data) => {8      setTodoCount(data.length)9    },10  })1112  return { todos, todoCount }13}
```
...
- `todos` will be `undefined` and length will be 0. That's the initial state while the Query is fetching. This is correct.
- `todos` will be an Array of length 5 and `todoCount` will be 0. That's the in-between render cycle where `useQuery` has already run (and so has `onSuccess`), but `setTodoCount` was scheduled. This is wrong because values are not in-sync.
- `todos` will be an Array of length 5 and `todoCount` will be 5. That is the final state and is correct again.
...
Now of course, the simple fix is to *derive state*. I have an old article on that topic as well, - [Don't over useState](https://tkdodo.eu/blog/dont-over-use-state). For this example, it merely means:

derive-state

```plain text
1export function useTodos() {2  const { data: todos } = useQuery({3    queryKey: ['todos', 'list'],4    queryFn: fetchTodos,5  })67  const todoCount = todos?.length ?? 089  return { todos, todoCount }10}
```
...
```plain text
1export function useTodos() {2  const { dispatch } = useDispatch()34  return useQuery({5    queryKey: ['todos', 'list'],6    queryFn: fetchTodos,7    onSuccess: (data) => {8      dispatch(setTodos(data))9    },10  })11}
```
...
```plain text
1export function useTodos(filters) {2  const { dispatch } = useDispatch()34  return useQuery({5    queryKey: ['todos', 'list', { filters }],6    queryFn: () => fetchTodos(filters),7    staleTime: 2 * 60 * 1000,8    onSuccess: (data) => {9      dispatch(setTodos(data))10    },11  })12}
```
...
```plain text
1export function useTodos(filters) {2  const { dispatch } = useDispatch()34  const query = useQuery({5    queryKey: ['todos', 'list', { filters }],6    queryFn: () => fetchTodos(filters),7    staleTime: 2 * 60 * 1000,8  })910  React.useEffect(() => {11    if (query.data) {12      dispatch(setTodos(query.data))13    }14  }, [query.data])1516  return query17}
```
...
[Theo - t3.gg@t3dotgg](https://twitter.com/t3dotgg/status/1647345483676024833)

[UPDATE: 27 instances of `onSuccess`, 4 are in queries, 3 are almost certainly sources of bugs](https://twitter.com/t3dotgg/status/1647345483676024833)

[Deprecate away 🙏](https://twitter.com/t3dotgg/status/1647345483676024833)

- Apr 15, 2023

That's why it's better to *not* have this API in the first place, and that's why we're removing it.

That's it for today. Feel free to reach out to me on [twitter](https://twitter.com/tkdodo) if you have any questions, or just leave a comment below. ⬇️