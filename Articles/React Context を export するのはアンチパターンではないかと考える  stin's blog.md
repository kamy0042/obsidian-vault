---
Created: 2021-02-21T22:27:00
URL: https://blog.stin.ink/articles/do-not-export-react-context
...
```plain text
const StringProvider: React.FC = ({ children }) => {  return <context.Provider value="another value">{children}</context.Provider>;};
```
...
const StringConsumer: React.VFC = () => {  const stringValue = useContext(context);
...
```plain text
export default function App() {  return (    <StringProvider>      <StringConsumer /> {/* another value と表示される */}    </StringProvider>  );}
```
...
const UserProvider: React.FC = ({ children }) => {  const [user, setUser] = useState<User | null>(null);
  useEffect(() => {    fetchUser().then(user => setUser(user));  }, []);
  return <userContext.Provider value={user}>{children}</userContext.Provider>;};
const UserInfo: React.VFC = () => {  const user = useContext(userContext);
  if (user === null) return <div>Loading...</div>;
  return (    <div>      <div>id: {user.id}</div>      <div>name: {user.name}</div>    </div>  );};
export default function App() {  return (    <UserProvider>      <UserInfo />    </UserProvider>  );}
```
...
const Greeting: React.VFC = () => {  const user = useContext(userContext);  return <p>こんにちは、{user.name}さん</p>;};
```
...
```plain text
export function useLoginUser(): User {  const user = useContext(userContext);
